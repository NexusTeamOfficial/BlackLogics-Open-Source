...
public final class FullStorage {
    ...
    private static final int VERSION = 3; // Version 3 for variables support
    ...
    private static void writeAll(
        ...
        try {
            ...
            Iterator<Map.Entry<String, ComponentData>> compIter = components.entrySet().iterator();
            while (compIter.hasNext()) {
                Map.Entry<String, ComponentData> entry = compIter.next();
                out.writeUTF(entry.getKey());
                ComponentData c = entry.getValue();
                out.writeInt(c.type);
    ...
    public static Object[] readAll() {
        ...
        try {
            ...
            Log.e("FullStorage", "READ ERROR (File might be corrupted)", e);
        } finally {
            try {
                if (in != null) in.close();
