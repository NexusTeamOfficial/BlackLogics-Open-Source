...
public class BlackLogReceiver extends BroadcastReceiver {
        ...
        try {
            ...
            if (writer != null) {
                try {
                    ...
                    Log.e("BlackLog", "Error closing writer", e);
                }
            }
        }
    }
    ...
    private void rotateLogs(File logDir) {
        File[] logFiles = logDir.listFiles(new java.io.FilenameFilter() {
            @Override
            public boolean accept(File dir, String name) {
                return name.startsWith(LOG_PREFIX) && name.endsWith(LOG_EXT);
            }
        });
    ...
    private void notifyUI(final String packageName, final String log, final int level, final long timestamp) {
        if (logListener != null) {
