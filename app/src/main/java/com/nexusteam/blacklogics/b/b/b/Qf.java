...
public class Qf {
    ...
    /**
     * Java 7 compatible saveBlockLogic.
     * Isme project_logic.bin system ka hi use kiya gaya hai consistency ke liye.
     */
    public static void saveBlockLogic(String activityName, String logic) {
        try {
            ...
            Map<String, String> actLogic;
            if (blocks.containsKey(activityName)) {
                actLogic = blocks.get(activityName);
            } else {
                actLogic = new HashMap<String, String>();
            }


            actLogic.put(activityName, logic); // Aapke code ke mutabiq activityName hi key hai
            blocks.put(activityName, actLogic);
