...
public class ProguardHandler {
    ...
    public ProguardHandler(String sc_id) {
        ...
        config_path =
            FileUtil.getExternalStorageDir() + "/.blacklogics/data/" + sc_id + "/proguard";
        fm_config_path =
            FileUtil.getExternalStorageDir() + "/.blacklogics/data/" + sc_id + "/proguard_fm";
    ...
    private static String createAndroidRules() {
        String rulePath =
            FileUtil.getExternalStorageDir() + "/.blacklogics/libs/android-proguard-rules.pro";

        if (!FileUtil.isExistFile(rulePath)) {
            FileUtil.writeFile(rulePath,
                "-dontusemixedcaseclassnames\n-dontskipnonpubliclibraryclasses\n-verbose\n\n-"
                    + "dontoptimize\n-dontpreverify\n\n-keepattributes *Annotation*\n-keep public "
                    + "class com.google.vending.licensing.ILicensingService\n-keep public class "
    ...
    private static String createDefaultRules(String sc_id) {
        String path = FileUtil.getExternalStorageDir() + "/.blacklogics/data/" + sc_id
            + "/proguard-rules.pro";
        ...
        if (FileUtil.isExistFile(config_path)) {
            try {
                HashMap<String, String> config =
                    new Gson().fromJson(FileUtil.readFile(config_path), hashMapStringStringType);
    ...
    public boolean libIsProguardFMEnabled(String library) {
        ...
        if (isProguardEnabled() && FileUtil.isExistFile(fm_config_path)) {
            ...
            try {
                enabled =
                    ((ArrayList<String>) new Gson().fromJson(configContent, arrayListStringType))
                        .contains(library);
                return enabled;
