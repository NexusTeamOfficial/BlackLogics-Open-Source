package com.nexusteam.sdk.resource;
...
import com.nexusteam.sdk.activity.SrcCodeEditor;
...
public class ManageResourceActivity extends Activity implements View.OnClickListener {
                ...
                for (String path : selections) {
                    try {
                        ...
                        SketchwareUtil.toastError(
                            "Couldn't import resource! [" + e.getMessage() + "]");
                    }
                }
                handleAdapter(temp);
                handleFab();
    ...
    private void goEdit(int position) {
        if (frc.listFileResource.get(position).endsWith("xml")) {
            ...
            intent.putExtra(
                "title", Uri.parse(frc.listFileResource.get(position)).getLastPathSegment());
                                ...
                                case "Edit with...":
                                    if (frc.listFileResource.get(position).endsWith("xml")) {
                                        ...
                                        intent.setDataAndType(
                                            Uri.fromFile(
                                                new File(frc.listFileResource.get(position))),
                                            "text/plain");
                                        startActivity(intent);
                                    } else {
                                        SketchwareUtil.toast("Only XML files can be edited");
