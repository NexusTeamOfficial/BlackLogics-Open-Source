package com.nexusteam.sdk.library.locallibrary;
        ...
        if (FileUtil.isExistFile(filePathUtil.getPathLocalLibrary(projectId))) {
            try {
                list = new Gson().fromJson(
                    FileUtil.readFile(filePathUtil.getPathLocalLibrary(projectId)),
    ...
    public ArrayList<String> getGenLocalLibrary() {
        ...
        for (String packageName : getPackageNames()) {
            if (!packageName.isEmpty()) {
                File projectGenFolder = new File(
                    "/storage/emulated/0", ".blacklogics/mysc/".concat(projectId).concat("/gen"));
    ...
    public String getJarLocalLibrary() {
        ...
        for (int i = 0, listSize = list.size(); i < listSize; i++) {
            ...
            if (jarPath instanceof String) {
                ...
                SketchwareUtil.toastError(
                    "Invalid JAR path of enabled Local library #" + i, Toast.LENGTH_LONG);
            }
        }
        ...
        for (int i = 0, listSize = list.size(); i < listSize; i++) {
            ...
            if (localLibrary.containsKey("resPath")) {
                ...
                if (resPath instanceof String) {
                    ...
                    SketchwareUtil.toastError(
                        "Invalid res/ folder path of enabled Local library #" + i,
                        Toast.LENGTH_LONG);
                }
            }
        }
