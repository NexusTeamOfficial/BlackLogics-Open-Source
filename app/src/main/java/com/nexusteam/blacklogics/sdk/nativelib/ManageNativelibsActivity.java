package com.nexusteam.sdk.nativelib;
...
public class ManageNativelibsActivity extends Activity implements View.OnClickListener {
    ...
    public void onBackPressed() {
        nativeLibrariesPath =
            nativeLibrariesPath.substring(0, nativeLibrariesPath.lastIndexOf("/"));
    ...
    private void createNewDialog() {
        ...
        root.findViewById(R.id.dialog_text_save).setOnClickListener(new View.OnClickListener() {
            @Override
        ...
        filePicker.setDialogSelectionListener(new DialogSelectionListener() {
            ...
            public void onSelectedFilePaths(String[] selections) {
                for (String path : selections) {
                    try {
                        FileUtil.copyDirectory(new File(path),
                            new File(nativeLibrariesPath + File.separator
                                + Uri.parse(path).getLastPathSegment()));
                    } catch (IOException e) {
                        SketchwareUtil.toastError(
                            "Couldn't import library! [" + e.getMessage() + "]");
                    }
                }
    ...
    private void showDialog(final String path) {
        ...
        final TextInputLayout filenameLayout =
            root.findViewById(R.id.dialoginputlayoutLinearLayout2);
            ...
            public void onClick(View saveButton) {
                ...
                if (!newName.isEmpty()) {
                    if (FileUtil.renameFile(path,
                            path.substring(0, path.lastIndexOf(File.separator)) + File.separator
                                + newName)) {
    ...
    private class CustomAdapter extends BaseAdapter {
        ...
        private int getImageRes(int position) {
            return FileUtil.isDirectory(getItem(position)) ? R.drawable.ic_folder_48dp
                                                           : R.drawable.file_96;
        }
    }
}