...
public class ViewCodeEditorActivity extends AppCompatActivity {
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		save.setOnClickListener(v -> {
			if (saveView()) {
				...
				// showMessage is already called inside saveView on failure
			}
		});
	...
	public boolean saveView() {
		try {
			...
			// 2. Project JSON ko load/update/save karein
    ...
    // --- JSON Utility Methods for your desired format ---
    
    // JSON file se data read karna
    private JSONArray readProjectJson() {
        try {
            // Project path ke hisaab se file path set karein
            String filePath = android.os.Environment.getExternalStorageDirectory() +
                "/.blacklogics/data/" + sc_id + "/root_layout.json";
            File file = new File(filePath);
            
            if (!file.exists()) {
                // Agar file exist nahi karti, to empty array return karein
                return new JSONArray();
            }
            
            // File content ko string mein padhein
            ...
            // JSON string ko JSONArray mein parse karein
    ...
    // JSON Array mein activity ka XML update karna
    private boolean updateActivityXmlInJson(String activityName, String xmlContent, JSONArray projectJsonArray) throws JSONException {
        ...
        for (int i = 0; i < projectJsonArray.length(); i++) {
            ...
            // Agar 'name' (layout/activity name) match karta hai
            if (activityObject.optString("name").equals(activityName)) {
                
                // XML content ko update karein. JSON lib khud hi escaping handle karta hai.
        ...
        if (!found) {
            // Agar activity nahi mili, to naya object bana kar add karein
    ...
    // Modified method: Updated JSON Array ko file mein save karna
    private boolean saveProjectJson(String activityName, String xmlContent) {
        try {
            ...
            // .toString() se compact JSON save hoga, jo aapke desired format se match karta hai.
