...
import com.besome.blacklogics.project.encryption.*;
...
public class CompilerAsyncTask extends AsyncTask<Project, String, CompilerResult> {
	...
	public void onPreExecute() {
		...
		if (context != null) {
			...
			// Back press automatic handle karega, manual code remove karo
			dialog.show();
			
			// Progress text directly update kar sakte ho
	...
	private void encryptResources() throws Exception {
		...
		// Encrypt the unsigned APK itself (no duplicate file)
		String unsignedApk = project.getOutputFile() + "/bin/gen.apk";
		
		// Directly apply encryption to gen.apk
