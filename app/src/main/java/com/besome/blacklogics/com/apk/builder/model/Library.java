...
public class Library {
		...
		for (File child : childs) {
			if (child.isDirectory()) {
				...
				if (classesJar.exists()) {
					...
					libraries.add(lib);
					System.out.println("Found library: " + lib.getName() + ", classes.jar: " + classesJar.getAbsolutePath());
				} else {
					System.out.println("No classes.jar found in: " + child.getAbsolutePath());
				}
			}
		}
		
		if (libraries.isEmpty()) {
			System.out.println("No valid libraries found in: " + file.getAbsolutePath());
		}
		return libraries;
	}
	...
	public List<File> getDexFiles() {
		...
		// If no .dex files are found, include .jar files as fallback
		if (files.isEmpty()) {
			for (File file : fileArr) {
				if (file.getName().endsWith(".jar")) {
					files.add(file);
					System.out.println("No .dex files found, including .jar file as fallback: " + file.getAbsolutePath());
				}
			}
			if (files.isEmpty()) {
				System.out.println("No .dex or .jar files found in library path: " + mPath.getAbsolutePath());
			}
		}
