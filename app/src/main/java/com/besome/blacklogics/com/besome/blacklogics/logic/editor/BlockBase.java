...
public class BlockBase extends RelativeLayout {
	...
	public BlockBase(Context var1, String var2, boolean var3) {
		...
		switch(var5) {
			...
			break;
			case 11:
			this.mShapeType = 1; // Same RectShape for 's.inputOnly'
			break;
		}
	...
	// BlockBase.java mein add karo
	
	/**
* Block ka color set karta hai
* @param color - ARGB color value
* @param shouldRedraw - Kya immediately redraw karna hai
*/	
	public void setBlockColor(int color) {
		this.mColor = color;
		if (this.mPaint != null) {
			this.mPaint.setColor(color); // Immediate paint update
		}
		if (isNeedRedraw()) {
			redraw(); // Layout refresh
		}
	}
	...
	/**
* Check karta hai ki redraw karna chahiye ya nahi
*/	
	private boolean isNeedRedraw() {
		return this.getParent() != null; // Agar parent hai toh redraw karo
	}
	
	/**
* Color string se set karta hai (hex format)
* @param hexColor - #RRGGBB format
*/	
	public void setBlockColorFromHex(String hexColor) {
		try {
			int color = Color.parseColor(hexColor);
			setBlockColor(color);
		} catch (IllegalArgumentException e) {
			// Invalid color, default use karo
			setBlockColor(this.defaultColor);
		}
	}
}
