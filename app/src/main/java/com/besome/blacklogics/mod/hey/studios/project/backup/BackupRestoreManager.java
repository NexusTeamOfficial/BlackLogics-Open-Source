/*
All copyrights reserved by NexusTeam & SmartIndiaGaming 

It's code part of open source project and source code available in public 

And first of fall this is also illigal file decompiled for original sketchware by sketchware pro modders 

I just used to my project and modify it

I just named for good filling and stability of codes.

@Thankyou Sketchware Pro
...
this.gc = gc;
	...
	public void backup(final String sc_id, final String project_name) {
		...
		MaterialAlertDialogBuilder dialogBuilder = new MaterialAlertDialogBuilder(act)
		.setIcon(com.besome.blacklogics.R.drawable.ic_backup)
		...
		LinearLayout.LayoutParams.MATCH_PARENT));
		int dip = (int) SketchwareUtil.getDip(act, 8);
		...
		MaterialCheckBox includeLocalLibraries = new MaterialCheckBox(act);
		...
		MaterialCheckBox includeUsedCustomBlocks = new MaterialCheckBox(act);
		...
		dialogBuilder.setView(checkboxContainer)
		.setPositiveButton("Back up", (dialog, which) -> {
			dialog.dismiss();
	...
	private static class RestoreAsyncTask extends AsyncTask<String, Integer, String> {
		...
		RestoreAsyncTask(WeakReference<Activity> activityWeakReference, String file, boolean restoreLocalLibraries) {
			this.activityWeakReference = activityWeakReference;
			this.file = file;
			//this.gc = gc;
		...
		@Override
		protected String doInBackground(String... params) {
			bm = new BackupFactory(getNewScId());
...
*/	
	private static int getNextProjectNumber() {
		File projectsDir = new File(Environment.getExternalStorageDirectory(), ".blacklogics/data/");
