...
public class WidgetAttributesManager {
    ...
    private void updateCache(String activityName, String widgetId, String attributeName, String value) {
        activityWidgetsCache.computeIfAbsent(activityName, k -> new HashMap<>())
                .computeIfAbsent(widgetId, k -> new HashMap<>())
                .put(attributeName, value);

        // Sync to your global widgetCustomAttributes (assuming it's widgetId -> attrs)
        ViewEditorFragmentActivity.widgetCustomAttributes.put(widgetId,
                activityWidgetsCache.get(activityName).get(widgetId));
    }
    ...
    public Map<String, Map<String, String>> getWidgetsForActivity(String activityName) {
        return activityWidgetsCache.getOrDefault(activityName, new HashMap<>());
    }
}