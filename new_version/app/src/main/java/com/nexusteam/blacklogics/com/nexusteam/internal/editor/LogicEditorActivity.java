...
public class LogicEditorActivity extends BaseAppCompatActivity implements fr, OnClickListener, OnTouchListener {
	...
	"/storage/emulated/0/.blacklogics/resources/block/My Block/block.json";
    ...
    final lr a;           // Validator
    final RadioGroup b;   // RadioGroup for type selection
    final EditText c;     // EditText for list name
    final CustomAlertDialog d;  // Dialog
    final LogicEditorActivity e; // Activity
    
    LogicEditorActivity$3(LogicEditorActivity activity, lr validator, 
                          RadioGroup group, EditText input, CustomAlertDialog dialog) {
        this.e = activity;
        this.a = validator;
        this.b = group;
        this.c = input;
    ...
    public void onClick(View v) {
        ...
        int listType = 1; // 1 = Number list (List<Double>)
        
        if (checkedId == com.nexusteam.blacklogics.R.id.rb_int) {
            listType = 1;   // List<Double>
        } else if (checkedId == com.nexusteam.blacklogics.R.id.rb_string) {
            listType = 2;   // List<String>
        } else if (checkedId == com.nexusteam.blacklogics.R.id.rb_map) {
            listType = 3;   // List<Map<String, Object>>
        }
        
        String listName = c.getText().toString().trim();
        
        if (listName.isEmpty()) {
            Toast.makeText(e.getApplicationContext(), 
        ...
        // STEP 1: Check if list already exists using e() method (returns boolean)
        lw storage = ma.a(e.f);
        if (storage.e(e.getJavaName(), listType, listName)) {
            Toast.makeText(e.getApplicationContext(), 
        ...
        // STEP 2: Store in runtime cache using b() method (adds list)
        storage.b(e.getJavaName(), listType, listName);
        
        // STEP 3: Store in BlockStorage permanently
        // Convert listType to storage type: 4=listInt, 5=listStr, 6=listMap
        int storageType = listType + 3;
        VariableData listData = new VariableData(storageType, listName, "[]");
        
        ArrayList<VariableData> variables = BlockStorage.loadVariables(e.getJavaName());
        if (variables == null) {
        ...
        for (VariableData vd : variables) {
            if (vd.name.equals(listName) && vd.type >= 4 && vd.type <= 6) {
                exists = true;
                break;
            }
        }
        ...
        if (!exists) {
            variables.add(listData);
            BlockStorage.saveVariables(e.getJavaName(), variables);
            
            // STEP 4: Store in ProjectLogicRepository for Java code generation
            String typeName = "";
            switch (listType) {
                case 1: typeName = "List<Double>"; break;
                case 2: typeName = "List<String>"; break;
                case 3: typeName = "List<Map<String, Object>>"; break;
            }
            ProjectLogicRepository.saveVariable(e.activityName, typeName, listName);
            
            Toast.makeText(e.getApplicationContext(), 
                "List '" + listName + "' added successfully", 
                Toast.LENGTH_SHORT).show();
        } else {
            Toast.makeText(e.getApplicationContext(), 
                "List already exists in storage!", 
                Toast.LENGTH_SHORT).show();
        }
...
class LogicEditorActivity$5 implements View.OnClickListener {
    final ViewGroup a;      // ViewGroup containing radio buttons
    final CustomAlertDialog b;  // Dialog
    final LogicEditorActivity c; // Activity
    
    LogicEditorActivity$5(LogicEditorActivity logicEditorActivity, 
                          ViewGroup viewGroup, CustomAlertDialog dialog) {
        this.c = logicEditorActivity;
        this.a = viewGroup;
        this.b = dialog;
    }
    ...
    public void onClick(View v) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (rb.isChecked()) {
                String listName = rb.getText().toString();
                int listType = (int) rb.getTag(); // Get stored list type (1,2,3)
                
                // ========== REMOVE LIST VARIABLE ==========
                
                // STEP 1: Remove from runtime cache using g() method
                lw storage = ma.a(c.f);
                storage.g(c.getJavaName(), listName);
                
                // STEP 2: Remove from BlockStorage permanently
                ArrayList<VariableData> variables = BlockStorage.loadVariables(c.getJavaName());
                if (variables != null) {
                    Iterator<VariableData> it = variables.iterator();
                    while (it.hasNext()) {
                        VariableData vd = it.next();
                        if (vd.name.equals(listName) && vd.type >= 4 && vd.type <= 6) {
                            it.remove();
                            break;
                        }
                    }
                    BlockStorage.saveVariables(c.getJavaName(), variables);
                }
                
                // STEP 3: Remove from ProjectLogicRepository
                int storageType = listType + 3; // 4,5,6
                ProjectLogicRepository.removeVariable(c.activityName, 
                    String.valueOf(storageType), listName);
                
                Toast.makeText(c.getApplicationContext(), 
                    "List '" + listName + "' removed successfully", 
                    Toast.LENGTH_SHORT).show();
	...
	class LogicEditorActivity$43 implements View.OnClickListener {
		...
		public void onClick(View view) {
			...
			if (checkedId == com.nexusteam.blacklogics.R.id.rb_boolean) {
				value = 0;
				typeName = "boolean";
			} else if (checkedId == com.nexusteam.blacklogics.R.id.rb_int) {
				value = 1;
				typeName = "double";
			} else if (checkedId == com.nexusteam.blacklogics.R.id.rb_string) {
				value = 2;
			...
			// save variable
			ProjectLogicRepository.saveVariable(e.activityName, typeName, text);
		...
		public void onClick(View v) {
			...
			for (int i = 0; i < childCount; i++) {
				...
				if (radio.isChecked()) {
					...
					// remove from project logic
					ProjectLogicRepository.removeVariable(activityName, String.valueOf(type), name);
					
					// remove from block storage
...
var13.into(var14);*/				
	...
	private void a(BlockBean var1, boolean var2) {
		...
		if (var10 != null) {
			...
			if (var1.isCustomBlock) {
				var10.z = var1.color;  // Color restore
			}
				...
				if (var11 != null) {
					...
					if (var3 > 0 && var11.charAt(0) == '@') {
						...
						if (this.h.equals("onBindCustomView")) {
							ViewBean var14 = ViewParser.getInstance().getViewBean(this.activityName, this.g);
							var8 = var9;
	...
	private void k() {
		...
		// 🔥 FIXED: View palette section - Built-in views se pehle try karo, nahi to ViewParser se lo
		if (this.h.equals("onBindCustomView")) {
			// Custom View case
			String customViewName = null;
			
			// पहले ViewParser से current view का data लो
			ViewBean currentView = ViewParser.getInstance().getViewBean(
			this.activityName, 
			this.g
			);
			
			Log.d("VIEW_PALETTE", "CustomView from parser: " + 
			(currentView != null ? currentView.customView : "null"));
			
			if (currentView != null) {
				customViewName = currentView.customView;
			}
			
			// Old method से fallback
			if (customViewName == null || customViewName.length() == 0) {
				customViewName = ma.a(this.f).e(getXmlName(), this.g).customView;
				Log.d("VIEW_PALETTE", "CustomView from ma: " + customViewName);
			}
			
			if (customViewName != null && customViewName.length() > 0) {
				// 🔥 IMPORTANT: Palette में section add करो
				this.a("Custom Views", -11184811);
				
				// Custom View के सभी views लो
				ArrayList<ViewBean> customViewBeans = ViewParser.getInstance().getAllBeans(
				this.activityName
				);
				
				if (customViewBeans != null) {
					Log.d("VIEW_PALETTE", "CustomView beans count: " + customViewBeans.size());
					
					var10 = customViewBeans.iterator();
					while(var10.hasNext()) {
						var14 = (ViewBean)var10.next();
						
						// 🔥 DEBUG: Check if view belongs to custom view
						if (var14.customView != null && var14.customView.equals(customViewName)) {
							
							// 🔥 FIXED: Pehle ViewBean.getViewTypeName() try karo
							String typeName = ViewBean.getViewTypeName(var14.type);
							
							// Agar empty hai toh ViewParser se lo
							if (typeName == null || typeName.isEmpty()) {
								typeName = ViewParser.getInstance().getViewXmlTag(
								this.activityName, var14.id
								);
								Log.d("VIEW_PALETTE", "Got from ViewParser: " + typeName + " for view: " + var14.id);
							}
							
							// Agar ab bhi empty hai toh default do
							if (typeName == null || typeName.isEmpty()) {
								typeName = "View";
							}
							
							Log.d("VIEW_PALETTE", "Adding custom view: " + var14.id + 
							" type: " + var14.type + " typeName: " + typeName);
							
							var11 = this.a(var14.id, "v", typeName, "getVar");
				...
				// Fallback to normal activity case
				// 🔥 IMPORTANT: Palette में section add करो
				this.a("Views", -11184811);
				
				// Current activity के सभी views लो
				ArrayList<ViewBean> activityBeans = ViewParser.getInstance().getAllBeans();
				
				if (activityBeans != null) {
					Log.d("VIEW_PALETTE", "Activity beans count: " + activityBeans.size());
					
					var10 = activityBeans.iterator();
					while(var10.hasNext()) {
						var14 = (ViewBean)var10.next();
						...
						Log.d("VIEW_PALETTE", "Adding view: " + var14.id + 
						" type: " + var14.type + " typeName: " + typeName);
					...
					Log.d("VIEW_PALETTE", "No activity beans found!");
				}
		...
		// First, load default/built-in components
		ArrayList<ComponentBean> defaultComps = ma.a(this.f).k(activityName);
		if (defaultComps == null) defaultComps = new ArrayList<ComponentBean>();
		
		for (int i = 0; i < defaultComps.size(); i++) {
			ComponentBean comp = defaultComps.get(i);
			View componentView = this.a(
			comp.componentId,
			"p",
			ComponentBean.getComponentTypeName(comp.type),
			"getVar"
			);
			componentView.setTag(comp.componentId);
		}
		
		// 🔥 FIXED: Custom components handling
		// Load from BlockStorage first (user added components)
		ArrayList<ComponentData> storedComps = BlockStorage.loadComponents(activityName);
		if (storedComps == null) storedComps = new ArrayList<ComponentData>();
		
		// Create a map to track which component types we've already added to avoid duplicates
		HashMap<String, Boolean> addedComponentTypes = new HashMap<String, Boolean>();
		
		// First, mark default component types as already added
		for (int i = 0; i < defaultComps.size(); i++) {
			ComponentBean comp = defaultComps.get(i);
			if (comp.typeName != null && !comp.typeName.isEmpty()) {
				addedComponentTypes.put(comp.typeName, true);
			}
		}
		
		// Now load custom components from ComponentManager
		ComponentManager cm = ComponentManager.getInstance();
		cm.refreshCustomComponents();
		List<ComponentBean> customComponentsFromJson = cm.getCustomComponents();
		
		// For debugging
		Log.d("COMPONENT_LOAD", "Custom components from JSON: " + customComponentsFromJson.size());
		
		// First, add all custom components from JSON that match stored instances
		for (int i = 0; i < customComponentsFromJson.size(); i++) {
			ComponentBean customComp = customComponentsFromJson.get(i);
			
			// Check if there are any stored instances of this component type
			boolean hasInstances = false;
			for (int j = 0; j < storedComps.size(); j++) {
				ComponentData data = storedComps.get(j);
				if (customComp.typeName != null && customComp.typeName.equals(data.extraData)) {
					hasInstances = true;
					break;
				}
			}
			
			// If there are instances AND we haven't added this type yet
			if (hasInstances && !addedComponentTypes.containsKey(customComp.typeName)) {
				Log.d("COMPONENT_LOAD", "Adding custom component type to palette: " + customComp.typeName);
				
				// Add the component type to palette (not instances yet)
				// The instances will be loaded from storedComps below
				addedComponentTypes.put(customComp.typeName, true);
			}
		}
		
		// Now add all stored component instances
		for (int i = 0; i < storedComps.size(); i++) {
			ComponentData data = storedComps.get(i);
			
			// Get full bean from ComponentManager to get typeName
			ComponentBean fullBean = cm.getComponentById(data.mainData);
			
			String typeName;
			if (fullBean != null && fullBean.typeName != null && !fullBean.typeName.isEmpty()) {
				typeName = fullBean.typeName; // JSON wala proper typeName
			} else {
				typeName = data.extraData; // fallback
			}
			
			if (typeName == null || typeName.isEmpty()) continue;
			
			Log.d("COMPONENT_LOAD", "Adding component instance: " + data.mainData + " type: " + typeName);
			
			View componentView = this.a(
			data.mainData,  // instance ID e.g. "fpP"
			"p",
			typeName,
			"getVar"
			);
			if (componentView == null) continue;
			componentView.setTag(data.mainData);
		}
	}
	...
	private void s() {
		...
		if (var11 != null) {
			...
			while(var10.hasNext()) {
				...
				var14 = this.a(var13);
				if (var13.isCustomBlock) {
					var14.z = var13.color;
					var14.isCustomBlock = true;
					var14.originalId = var13.originalId;
					var14.setSpec(var13.spec);
				}
	...
	private void t() {
		...
		// ── 1. Imports special case ───────────────────────────────────────────
		if ("Imports".equals(h)) {
			ProjectLogicRepository.setImports(activityName, getSourceCode());
			BlockStorage.save(
		...
		// ── 2. Resolved source code ───────────────────────────────────────────
		String sourceCode = getSourceCode();

		// ── 3. System event (events.json) check ───────────────────────────────
		// g = widget/component id (e.g. "button1", "WebView"), h = event name (e.g. "onClick", "onProgressChanged")
		// Agar h kisi SystemEvent ke naam se match karta hai toh uska code template
		// %s placeholder mein sourceCode inject karke project store mein save karo.
		com.nexusteam.blacklogics.utils.EventUtils.SystemEvent sysEvent =
			com.nexusteam.blacklogics.utils.EventUtils.findSystemEventByName(h);

		if (sysEvent != null) {
			// Template mein %s / %1$s = user ka logic (sourceCode)
			String resolvedCode = sysEvent.buildCode(sourceCode);

			// Project event store mein save karo (activity_eventName key se)
			com.nexusteam.blacklogics.utils.EventUtils.saveSystemEventToProject(
				this.f,        // sc_id
				sysEvent,
				activityName,
				resolvedCode
			...
			// ProjectLogicRepository mein bhi sync karo taaki JavaCodeBuilder
			// getBlockLogicForEvent() se yeh logic padh sake
			Qf.saveBlockLogicForEvent(activityName, h, sourceCode);

		} else if (widgetId != null && !widgetId.isEmpty()) {
			// ── 4. Widget event (onClick, onLongClick etc.) ───────────────────
			Qf.saveBlockLogicForWidget(activityName, widgetId, sourceCode);

		} else if (type != null && !type.isEmpty()) {
			// ── 5. Typed event (component/lifecycle event string) ─────────────
			Qf.saveBlockLogicForEvent(activityName, type, sourceCode);

		} else {
			// ── 6. Default — main initializeLogic block ───────────────────────
			Qf.saveBlockLogic(activityName, sourceCode);
		}

		// ── 7. Debug dump + BlockStorage persist ─────────────────────────────
		FileUtil.writeFile(
			FileUtil.getExternalStorageDir().concat("code/code.txt"),
			sourceCode
		);
	...
	public void a(int var1, int var2) {
		...
		switch(var1) {
			...
			this.b(" ", "setTitle");
			if (ma.a(this.f).g(activityName, 1)) {
			...
			if (ma.a(this.f).g(activityName, 2)) {
				this.b("s", "fileGetData");
				this.b(" ", "fileSetData");
				this.b(" ", "fileRemoveData");
			}
			
			if (ma.a(this.f).g(activityName, 3)) {
			...
			if (ma.a(this.f).g(activityName, 4)) {
				this.b(" ", "vibratorAction");
			}
			
			if (ma.a(this.f).g(activityName, 5)) {
				this.b("c", "timerAfter");
				this.b("c", "timerEvery");
				this.b(" ", "timerCancel");
			}
			
			if (ma.a(this.f).g(activityName, 7)) {
			...
			if (ma.a(this.f).g(activityName, 8)) {
			...
			if (ma.a(this.f).g(activityName, 9)) {
			...
			if (ma.a(this.f).g(activityName, 10)) {
			...
			if (ma.a(this.f).g(activityName, 6)) {
				this.b(" ", "firebaseAdd");
				this.b(" ", "firebasePush");
			...
			if (ma.a(this.f).g(activityName, 12)) {
			...
			if (ma.a(this.f).g(activityName, 21)) {
...
this.startActivity(var2);*/		
	...
	public void d(fn var1) {
		...
		// 🔥 PEHLE CUSTOM COMPONENTS CHECK KARO
		ComponentManager cm = ComponentManager.getInstance();
		cm.refreshCustomComponents();
		List<ComponentBean> customComponents = cm.getCustomComponents();
		
		boolean isCustomComponent = false;
		String customComponentType = "";
		
		// Pehle BlockStorage se check karo - koi stored component is menuName se match karta hai?
		ArrayList<ComponentData> storedComps = BlockStorage.loadComponents(this.activityName);
		
		for (int j = 0; j < storedComps.size(); j++) {
			ComponentData data = storedComps.get(j);
			// data.extraData = typeName (exactly as saved from component.json)
			if (data.extraData != null && data.extraData.equals(var1.getMenuName())) {
				isCustomComponent = true;
				customComponentType = data.extraData;
				break;
			}
		}
		
		// Agar direct match nahi mila, ComponentManager se bhi check karo
		if (!isCustomComponent) {
			List<ComponentBean> customComponents1 = cm.getCustomComponents();
			for (int i = 0; i < customComponents1.size(); i++) {
				ComponentBean comp = customComponents1.get(i);
				if (comp.typeName != null && comp.typeName.equals(var1.getMenuName())) {
					isCustomComponent = true;
					customComponentType = comp.typeName;
					break;
				}
			}
		}
		...
		if (isCustomComponent) {
			var7.a("Select " + customComponentType);
			var5 = new ArrayList();
			
			// Sirf matching type ke instances add karo
			for (int j = 0; j < storedComps.size(); j++) {
				ComponentData data = storedComps.get(j);
				if (data.extraData != null && data.extraData.equals(customComponentType)) {
					var5.add(data.mainData); // "fpP", "dk" etc instance names
				}
			}
		} else if (var4) {
			...
			var5 = ma.a(this.f).g(this.activityName);
			...
			var5 = ma.a(this.f).e(this.activityName, 1);
			...
			var5 = ma.a(this.f).e(this.activityName, 2);
			...
			var5 = ma.a(this.f).e(this.activityName, 3);
			...
			var5 = ma.a(this.f).e(this.activityName, 4);
			...
			var5 = ma.a(this.f).e(this.activityName, 5);
			...
			var5 = ma.a(this.f).e(this.activityName, 6);
			...
			var5 = ma.a(this.f).e(this.activityName, 12);
			...
			var5 = ma.a(this.f).e(this.activityName, 14);
			...
			var5 = ma.a(this.f).e(this.activityName, 7);
			...
			var5 = ma.a(this.f).e(this.activityName, 8);
			...
			var5 = ma.a(this.f).e(this.activityName, 9);
			...
			var5 = ma.a(this.f).e(this.activityName, 10);
			...
			var5 = ma.a(this.f).e(this.activityName, 11);
			...
			var5 = ma.a(this.f).e(this.activityName, 13);
			...
			var5 = ma.a(this.f).e(this.activityName, 15);
			...
			var5 = ma.a(this.f).e(this.activityName, 16);
			...
			var5 = ma.a(this.f).e(this.activityName, 17);
			...
			var5 = ma.a(this.f).e(this.activityName, 18);
			...
			var5 = ma.a(this.f).e(this.activityName, 19);
			...
			var5 = ma.a(this.f).e(this.activityName, 20);
			...
			var5 = ma.a(this.f).e(this.activityName, 21);
	...
	public void e(fn var1) {
		...
		// 🔥 1. XML name decide karo
		String xmlName = this.activityName;
		//    Toast.makeText(this, "Activity XML: " + xmlName, Toast.LENGTH_SHORT).show();
		
		if (this.h.equals("onBindCustomView")) {
			
			//Toast.makeText(this, "CustomView mode", Toast.LENGTH_SHORT).show();
			
			ViewBean currentView = ViewParser.getInstance().getViewBean(xmlName, this.g);
			
			/*    if (currentView != null) {
Toast.makeText(this, "CurrentView found: " + currentView.id, Toast.LENGTH_SHORT).show();
} else {
Toast.makeText(this, "CurrentView NULL", Toast.LENGTH_SHORT).show();
}*/			
			
			if (currentView != null && currentView.customView != null && currentView.customView.length() > 0) {
				
				xmlName = ProjectFileBean.getXmlName(currentView.customView);
				//    Toast.makeText(this, "Custom XML: " + xmlName, Toast.LENGTH_SHORT).show();
				
			} else {
				
				//Toast.makeText(this, "Fallback method", Toast.LENGTH_SHORT).show();
				
				ViewBean var9 = ma.a(this.f).e(getXmlName(), this.g);
				
				if (var9 != null && var9.customView != null) {
					xmlName = ProjectFileBean.getXmlName(var9.customView);
					//    Toast.makeText(this, "Fallback XML: " + xmlName, Toast.LENGTH_SHORT).show();
				}
			}
		}
		
		var7.a(StringResourceManager.a().a(this.getApplicationContext(),
		com.nexusteam.blacklogics.R.string.logic_editor_title_select_view));
		
		// 🔥 2. View type
		String viewType = var1.getClassInfo().a();
		//Toast.makeText(this, "ViewType: " + viewType, Toast.LENGTH_SHORT).show();
		
		// 🔥 3. Views list
		ArrayList<Pair<Integer, String>> viewList =
		ViewParser.getInstance().getViewsByType(xmlName, viewType);
		
		if (viewList == null) {
			//    Toast.makeText(this, "ViewParser NULL", Toast.LENGTH_SHORT).show();
		} else {
			//    Toast.makeText(this, "ViewParser size: " + viewList.size(), Toast.LENGTH_SHORT).show();
		}
		
		// fallback
		if (viewList == null || viewList.isEmpty()) {
			
			//    Toast.makeText(this, "Using fallback ma()", Toast.LENGTH_SHORT).show();
			
			viewList = ma.a(this.f).c(xmlName, viewType);
			
			if (viewList != null) {
				//    Toast.makeText(this, "Fallback size: " + viewList.size(), Toast.LENGTH_SHORT).show();
			}
		}
		
		// 🔥 4. Radio buttons
		Iterator var11 = viewList.iterator();
		while (var11.hasNext()) {
			
			Pair var12 = (Pair) var11.next();
			
			//    Toast.makeText(this, "View: " + var12.second, Toast.LENGTH_SHORT).show();
			
			RadioButton var13 = this.d(
			ViewBean.getViewTypeName((Integer) var12.first),
			(String) var12.second
			);
		...
		var7.a(StringResourceManager.a().a(this.getApplicationContext(),
		com.nexusteam.blacklogics.R.string.common_word_select),
		new LogicEditorActivity$18(this, var8, var1, var7));
		
		var7.b(StringResourceManager.a().a(this.getApplicationContext(),
		com.nexusteam.blacklogics.R.string.common_word_cancel),
		new LogicEditorActivity$19(this, var7));
			...
			if (var1.getTag() != null) {
				if (var1.getTag().equals("variableAdd")) {
					...
					if (var1.getTag().equals("blockAdd")) {
						...
						var3.putExtra("activityName", this.activityName);
...
this.startActivityForResult(var3, 464);*/						
	...
	public void onCreate(Bundle var1) {
		...
		ViewParser.getInstance().init(this.f, this.activityName);
		
		// 2. Load data - सब हो जाएगा
		ArrayList<ViewBean> beans = ViewParser.getInstance().loadBeanData();
	...
	private void loadComponenetsIntoMemory() { 
		...
		for (String activityName : components.keySet()) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				String key = data.type + "_" + data.mainData + "_" + data.extraData;
	...
	protected void onPostCreate(@Nullable Bundle var1) {
		...
		String var8;
		String safeH = (this.h != null) ? this.h : "";
		String safeG = (this.g != null) ? this.g : "";
		
		ComponentManager.getInstance().refreshCustomComponents();
		loadFunctionsIntoMemory();
		loadComponenetsIntoMemory();
		loadVariablesIntoMemory();
		
		if (safeH.equals("moreBlock")) {
		    var8 = ma.a(this.f).i(this.getJavaName(), safeG);
			String var5 = StringResourceManager.a()
			.a(this.getApplicationContext(),
			com.nexusteam.blacklogics.R.string.root_spec_common_define);
			this.i = var5 + " " + var8;
		} else if (safeG.equals("_fab")) {
			this.i = StringResourceManager.a().a(this.getApplicationContext(), "fab", safeH);
		} else {
			this.i = StringResourceManager.a().a(this.getApplicationContext(), safeG, safeH);
		}
		
		// If i is null or empty, set fallback
		if (this.i == null || this.i.isEmpty()) {
			String fallback = StringResourceManager.a()
			.a(this.getApplicationContext(),
			com.nexusteam.blacklogics.R.string.root_spec_common_define);
			this.i = fallback + " " + safeG + " " + safeH;
		}
		...
		ArrayList<BlockBean> saved = BlockStorage.load(
		this.g + "_" + this.h + "_" + this.activityName
		);
		...
		if (saved != null && !saved.isEmpty()) {
			...
			// ✅ CUSTOM BLOCKS RESTORE KARO
			/*  for (BlockBean bean : saved) {
if (bean.isCustomBlock) {
fm block = this.e.c(bean.id);
if (block != null) {
block.z = bean.color;  // Color restore
block.isCustomBlock = true;
block.originalId = bean.originalId;
block.setSpec(bean.spec);  // Spec restore
}
}
}*/			
		}
		
		this.s();  // Blocks create honge
	...
	public boolean onTouch(View view, MotionEvent motionEvent) {
		...
		if (actionMasked == 0) {
			...
			this.p = view2;
			Log.d("BLOCK_TOUCH", "ACTION_DOWN block=" + view2);
			return true;
		}
...
this.J.send(eventBuilder.build());*/			
			Log.d("BLOCK_TOUCH", "ACTION_UP drop detected");
			this.p = null;
			this.U.removeCallbacks(this.V);
			if (!this.x) {
			...
			if (!this.s.getAllow()) {
				...
				if (fmVar7.getBlockType() == 1) {
					...
					Log.d("BLOCK_TOUCH", "Generated beans size=" + a5.size());
					if (a5.size() > 0) {
						Log.d("BLOCK_TOUCH", "Block list empty!");
						...
						Log.d("BLOCK_TOUCH", "Adding block to workspace");
						lu.a(this.f).a(h(), a5, this.y[0] - iArr6[0], this.y[1] - iArr6[1], clone4, blockBean3);
						a();
					}
					Log.d("BLOCK_TOUCH", "CUSTOM BLOCK DROP type2 detected");
				} else {
					this.e.a(fmVar7, 0);
...
return getJavaName();  // "MainActivity.java"
	...
	private void loadCustomBlocks(int paletteIndex, int defaultColor) {
		try {
			Log.d("CUSTOM_BLOCK", "Loading custom blocks...");
			
			BufferedReader br = new BufferedReader(new FileReader(BLOCK_PATH));
			...
			Log.d("CUSTOM_BLOCK", "Total blocks in JSON = " + arr.length());
			...
			for (int i = 0; i < arr.length(); i++) {
				...
				int palette = obj.getInt("palette");
				
				Log.d("CUSTOM_BLOCK", "Block Found → name=" + name + " spec=" + spec);
				...
				if (palette == paletteIndex) {
					Log.d("CUSTOM_BLOCK", "Palette matched → creating block");
					
					// Use PaletteBlock's a() method to create block
					fm block = (fm) this.c.a(spec, type, code);
					
					// Set all important fields
					block.setSpec(spec);
					block.z = color;
					block.w = type;
					block.b = code;
					
					// CRITICAL: Use NEGATIVE ID to identify custom blocks
					// This will be used in fq.c() to preserve data
					int uniqueId = -2000 - i;  // Negative IDs for palette blocks
					block.setTag(uniqueId);
					
					block.setClickable(true);
					block.setOnTouchListener(this);
					
					block.setBlockType(1);  // Palette block
					
					// Force refresh to create parameter blocks
					block.b();  // This calls fm.b() which positions all children
					
					Log.d("CUSTOM_BLOCK", "Block created successfully → id=" + uniqueId + 
					" opcode=" + block.b + " type=" + block.w + " spec=" + block.a +
					" color=" + block.z);
				}
			}
			
			Log.d("CUSTOM_BLOCK", "Custom block loading finished");
			
		} catch (Exception e) {
			Log.e("CUSTOM_BLOCK", "ERROR = " + e.toString());
		}
	}
