...
import com.nexusteam.sdk.project.library.LibraryDownloader;
import com.nexusteam.sdk.util.Helper;
import java.util.ArrayList;
import java.util.Collections;
import java.util.HashMap;

public class ManageLocalLibraryActivity
    extends Activity implements View.OnClickListener, LibraryDownloader.OnCompleteListener {
    ...
    public void initToolbar() {
        ...
        ImageView back_icon = (ImageView) findViewById(R.id.ig_toolbar_back);
        Helper.applyRippleToToolbarView(back_icon);
        back_icon.setOnClickListener(Helper.getBackPressedClickListener(this));
        ImageView import_library_icon = (ImageView) findViewById(R.id.ig_toolbar_load_file);
    ...
    public void onClick(View v) {
        new AlertDialog.Builder(this)
            ...
            .setMessage("Would you like to use Dx or D8 to dex the library?\nD8 supports Java 8, "
                + "whereas Dx does not. Limitation: D8 only works on Android 8 and above.")
            .setPositiveButton("D8",
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.search_perm).setVisibility(8);
        initToolbar();
        loadFiles();
    }
    ...
    private void loadFiles() {
        ...
        lookup_list.clear();
        local_libs_path = FileUtil.getExternalStorageDir().concat("/.blacklogics/libs/local_libs/");
        local_lib_file = FileUtil.getExternalStorageDir()
        ...
        if (!FileUtil.isExistFile(local_lib_file) || FileUtil.readFile(local_lib_file).equals("")) {
            ...
            project_used_libs = (ArrayList<HashMap<String, Object>>) new Gson().fromJson(
                FileUtil.readFile(local_lib_file), Helper.TYPE_MAP_LIST);
        }
    ...
    public class LibraryAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            final CheckBox checkBox = (CheckBox) convertView.findViewById(R.id.checkbox_content);
            ...
            checkBox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                ...
                public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                    ...
                    if (FileUtil.isExistFile(local_libs_path.concat(checkBox.getText().toString())
                                .concat("/config"))) {
                        hashMap.put("packageName",
                            FileUtil.readFile(local_libs_path.concat(checkBox.getText().toString())
                                    .concat("/config")));
                    }
                    if (FileUtil.isExistFile(
                            local_libs_path.concat(checkBox.getText().toString()).concat("/res"))) {
                        hashMap.put("resPath",
                            local_libs_path.concat(checkBox.getText().toString()).concat("/res"));
                    }
                    if (FileUtil.isExistFile(local_libs_path.concat(checkBox.getText().toString())
                                .concat("/classes.jar"))) {
                        hashMap.put("jarPath",
                            local_libs_path.concat(checkBox.getText().toString())
                                .concat("/classes.jar"));
                    }
                    if (FileUtil.isExistFile(local_libs_path.concat(checkBox.getText().toString())
                                .concat("/classes.dex"))) {
                    ...
                    if (FileUtil.isExistFile(local_libs_path.concat(checkBox.getText().toString())
                                .concat("/AndroidManifest.xml"))) {
                        hashMap.put("manifestPath",
                            local_libs_path.concat(checkBox.getText().toString())
                                .concat("/AndroidManifest.xml"));
                    }
                    if (FileUtil.isExistFile(local_libs_path.concat(checkBox.getText().toString())
                                .concat("/proguard.txt"))) {
                        hashMap.put("pgRulesPath",
                            local_libs_path.concat(checkBox.getText().toString())
                                .concat("/proguard.txt"));
                    }
                    if (FileUtil.isExistFile(local_libs_path.concat(checkBox.getText().toString())
                                .concat("/assets"))) {
                        hashMap.put("assetsPath",
                            local_libs_path.concat(checkBox.getText().toString())
                                .concat("/assets"));
                    }
                    if (!isChecked) {
                        ...
                        while (n < project_used_libs.size()) {
                            if (project_used_libs.get(n).get("name").toString().equals(
            ...
            lookup_list =
                new Gson().fromJson(FileUtil.readFile(local_lib_file), Helper.TYPE_MAP_LIST);
            n = 0;
            while (n < lookup_list.size()) {
                checkBox.setChecked(false);
                if (checkBox.getText().toString().equals(
                        lookup_list.get(n).get("name").toString())) {
                    checkBox.setChecked(true);
                }
                n = n + 1;
            }
            ((ImageView) convertView.findViewById(R.id.img_delete))
                        ...
                        popupMenu.setOnMenuItemClickListener(
                            new PopupMenu.OnMenuItemClickListener() {
                                ...
                                public boolean onMenuItemClick(MenuItem menuItem) {
                                    FileUtil.deleteFile(
                                        local_libs_path.concat(checkBox.getText().toString()));
                                    bB.a(ManageLocalLibraryActivity.this, "Deleted successfully", 0)
                                        .show();
                                    loadFiles();
                                    return true;
                                }
