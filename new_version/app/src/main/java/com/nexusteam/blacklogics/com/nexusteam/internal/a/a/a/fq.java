...
public fm c(fm var1, int var2, int var3) {
    ...
    if (var1.getBlockType() == 1) {
        ...
        // ALWAYS create unique runtime id
        int runtimeId = this.a++;

        String tagString = String.valueOf(var1.getTag());
        boolean isCustomBlock = tagString.startsWith("-");

        int originalId = 0;
        if (isCustomBlock) {
            try {
                originalId = Integer.parseInt(tagString);
            } catch (Exception e) {
                originalId = 0;
            }
        }

        // create block with UNIQUE runtime id
        var5 = new fm(var6, runtimeId, var1.a, var1.w, var1.x, var1.b);

        // copy color
        var5.z = var1.z;

        if (isCustomBlock) {
            var5.isCustomBlock = true;
            var5.originalId = originalId;
        }

        var5.setSpec(var1.a);

        // copy parameters safely
       if (var1.o != null && !var1.o.isEmpty()) {

    for (int i = 0; i < var1.o.size(); i++) {

        View child = (View) var1.o.get(i);

        if (child instanceof fn) {

            fn sourceFn = (fn) child;

            if (i < var5.o.size()) {

                fn targetFn = (fn) var5.o.get(i);

                targetFn.setArgValue(sourceFn.getArgValue());
            }
        }
    }
}
